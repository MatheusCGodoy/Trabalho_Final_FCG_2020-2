...
void main()
{
    ...
    float i=0;
    /*
    if(lambert == 0) // se dot(n,l) < 0
        i = 1.0f; //ligado/desligado
    else // senão
        i = dot(n,l);
    */

    /*
    if(dot(n,l) < 0) // se dot(n,l) < 0
        i = -dot(n,l); // usar o próprio valor para gradualmente aumentar/reduzir a "intensidade" da noite
    else // senão
        i = dot(n,l);
    */
    //ou
    ...
    color = Kd1*((i - lambert) + 0.1) + Kd0*(lambert + 0.01);
